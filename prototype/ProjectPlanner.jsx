...
import {
  Briefcase, ShoppingCart, LayoutGrid, Newspaper, MousePointerClick, HeartHandshake,
  Mail, CalendarClock, CreditCard, FileText, Globe, Search, BarChart3, Lock, Send,
  ArrowLeft, ArrowRight, Check, Clock, Sparkles, Pencil, CheckCircle2, Minus, Plus, X, Layers,
} from "lucide-react";

/* ---------------------------------- data ---------------------------------- */

const TYPES = [
  { id: "business", label: "Business Website", desc: "A professional home for your company", base: 2500, Icon: Briefcase },
  { id: "ecommerce", label: "E-commerce Store", desc: "Sell products online", base: 4000, Icon: ShoppingCart },
  { id: "portfolio", label: "Portfolio", desc: "Showcase your work", base: 1500, Icon: LayoutGrid },
  { id: "blog", label: "Blog / News", desc: "Publish content regularly", base: 2000, Icon: Newspaper },
  { id: "landing", label: "Landing Page", desc: "One focused page for a campaign", base: 800, Icon: MousePointerClick },
  { id: "nonprofit", label: "Non-Profit", desc: "Tell your cause's story, raise support", base: 2200, Icon: HeartHandshake },
];

const PAGE_OPTS = [
  { id: "s1", label: "Up to 5 pages", desc: "A focused, essential site", amount: 0 },
...
const PRODUCT_OPTS = [
  ...
  { id: "p2", label: "26 – 100 products", desc: "A growing store", amount: 800 },
  { id: "p3", label: "100+ products", desc: "A large catalog", amount: 2000 },
];
const SIZE_OPTS = [
  { id: "simple", label: "Just the essentials", desc: "One clear screen", amount: 0 },
  { id: "standard", label: "A few sections", desc: "Features, proof, a call to action", amount: 400 },
  { id: "detailed", label: "Long-form page", desc: "Detailed, scroll-rich storytelling", amount: 900 },
];

const DESIGNS = [
  { id: "minimal", label: "Clean & minimal", desc: "Calm, spacious, content-first", amount: 0 },
  { id: "corporate", label: "Corporate & professional", desc: "Structured and trustworthy", amount: 400 },
  { id: "bold", label: "Bold & distinctive", desc: "Strong type, color, motion", amount: 600 },
  { id: "custom", label: "Fully custom", desc: "Designed from scratch, uniquely yours", amount: 1500 },
];

const FEATURES = [
  { id: "forms", label: "Contact forms", desc: "Let visitors reach you", amount: 150, Icon: Mail },
  { id: "booking", label: "Booking / scheduling", desc: "Take appointments online", amount: 800, Icon: CalendarClock },
  { id: "payments", label: "Online payments", desc: "Accept cards & checkout", amount: 900, Icon: CreditCard },
  { id: "cms", label: "Blog / CMS", desc: "Edit content yourself", amount: 700, Icon: FileText },
  { id: "multilingual", label: "Multilingual", desc: "Reach more than one language", amount: 0, Icon: Globe },
  { id: "seo", label: "SEO setup", desc: "Get found on Google", amount: 400, Icon: Search },
  { id: "analytics", label: "Analytics", desc: "See how visitors behave", amount: 200, Icon: BarChart3 },
  { id: "membership", label: "Accounts / login", desc: "Members-only areas", amount: 1200, Icon: Lock },
  { id: "newsletter", label: "Newsletter", desc: "Collect & email subscribers", amount: 200, Icon: Send },
];

const TIMELINES = [
  { id: "flexible", label: "Flexible", desc: "No fixed deadline", note: "Best value" },
...
function computeEstimate(s) {
  ...
  if (s.content === "help") items.push({ key: "content", label: "Copywriting & content help", amount: 700 });
  ...
  (s.features || []).forEach((fid) => {
    ...
    if (fid === "multilingual") {
      ...
      items.push({ key: "multilingual", label: "Multilingual · " + langs + " languages", amount: (langs - 1) * 450 });
    } else {
...
function canContinue(step, s) {
  switch (step) {
    ...
    case 2: {
      const scopeOK = s.type === "landing" ? !!s.size : s.type === "ecommerce" ? !!s.products : !!s.pages;
      return scopeOK && !!s.content;
    }
    case 3: return !!s.design;
    case 4: return true;
    case 5: return !!s.timeline && !!s.budget;
    case 6: return s.name.trim().length > 0 && isEmail(s.email);
    default: return true;
  }
}
...
function useCountUp(target) {
  ...
  useEffect(() => {
    ...
    const start = ref.current, end = target;
    if (reduce || start === end) { ref.current = end; setVal(end); return; }
...
function OptionCard({ active, onClick, Icon, label, desc, tag, multi }) {
  return (
    <button
      ...
      aria-pressed={active}
    >
      {Icon && <span className="optIcon"><Icon size={20} strokeWidth={1.9} /></span>}
      ...
      <span className={"tick" + (multi ? " sq" : "")}>{active && <Check size={13} strokeWidth={3} />}</span>
    </button>
  );
}
      ...
      <div className="panelInner">
        ...
        {hasType ? (
          <>
            ...
            <div className="estNote">Working ballpark · updates as you choose</div>
            <div className="rule" />
            <ul className="lines">
              {est.items.map((it) => (
                <li key={it.key} className={"line" + (it.key === "rush" ? " rush" : "")}>
                  <span className="lineLabel">{it.label}</span>
                  <span className="lineAmt">{it.amount === 0 ? "Included" : "+" + fmt(it.amount)}</span>
                </li>
              ))}
            </ul>
            {midBand && (
          ...
          <div className="estEmpty">
            ...
            <p>Pick a website type to start building your estimate. Each choice updates the total in real time.</p>
          </div>
        )}
        <div className="estFoot">A ballpark from your choices. Your final quote is fixed after we talk — no surprises.</div>
      </div>
    </div>
  );
}
...
export default function ProjectPlanner() {
  ...
  const [s, setS] = useState({
    type: "", pages: "", products: "", size: "", content: "",
    design: "", features: [], languages: 2, timeline: "", budget: "",
  ...
  useEffect(() => {
    if (step === 5 && !s.budget && hasType) set({ budget: bandFor(est.total).id });
  ...
  const scopeOpts = s.type === "landing" ? SIZE_OPTS : s.type === "ecommerce" ? PRODUCT_OPTS : PAGE_OPTS;
  const scopeKey = s.type === "landing" ? "size" : s.type === "ecommerce" ? "products" : "pages";
  const scopeTitle = s.type === "landing" ? "How much goes on the page?"
    : s.type === "ecommerce" ? "How many products will you sell?"
    : "How big is the site?";

  const toggleFeature = (id) =>
    set({ features: s.features.includes(id) ? s.features.filter((f) => f !== id) : [...s.features, id] });

  const restart = () => {
    setDone(false); setStep(1);
    setS({ type: "", pages: "", products: "", size: "", content: "", design: "", features: [], languages: 2, timeline: "", budget: "", name: "", email: "", company: "", message: "" });
  };
    ...
    <div className="root">
      ...
      <header className="topbar">
        <div className="brand">
          ...
          <span className="brandText">DR Web Studio <span className="brandSub">· Project Planner</span></span>
        </div>
        {!done && <div className="timeHint"><Clock size={13} strokeWidth={2.1} /> About 2 minutes</div>}
      </header>

      {done ? (
        <Confirmation s={s} est={est} low={est.low} high={est.high} onRestart={restart} />
      ) : (
        <>
          {/* progress */}
          <div className="progress">
            <div className="segs">
              {Array.from({ length: TOTAL_STEPS }).map((_, i) => {
                ...
                return (
                  <button key={n} className={"seg " + state} onClick={() => goto(n)}
                    disabled={n >= step} aria-label={"Step " + n} />
                );
              })}
            </div>
            <div className="progLabel">Step {step} of {TOTAL_STEPS}</div>
          </div>
          ...
          <div className="layout">
            <main className="main">
              <div className="stepCard" key={step}>
                {step === 1 && (
                  <>
                    <StepHead kicker="01 · Project" title="What are you building?" sub="Pick the type that fits best — you can refine the details next." />
                    <div className="grid2">
                      {TYPES.map((t) => (
                        <OptionCard key={t.id} active={s.type === t.id} Icon={t.Icon} label={t.label} desc={t.desc}
                          onClick={() => set({ type: t.id, pages: "", products: "", size: "" })} />
                      ))}
                    </div>
                  </>
                )}
                ...
                {step === 2 && (
                  <>
                    <StepHead kicker="02 · Scope" title={scopeTitle} sub="An honest range is fine — we'll confirm the exact scope together." />
                    <div className="stack">
                      {scopeOpts.map((o) => (
                        <OptionCard key={o.id} active={s[scopeKey] === o.id} label={o.label} desc={o.desc}
                          tag={o.amount === 0 ? "Included" : "+" + fmt(o.amount)}
                          onClick={() => set({ [scopeKey]: o.id })} />
                      ))}
                    </div>
                    <div className="subhead">Is your content ready?</div>
                    <div className="grid2">
                      <OptionCard active={s.content === "ready"} Icon={CheckCircle2} label="Yes, it's ready"
                        desc="I have my text and images" onClick={() => set({ content: "ready" })} />
                      <OptionCard active={s.content === "help"} Icon={Pencil} label="I'll need help"
                        desc="Copywriting & content support" tag="+$700" onClick={() => set({ content: "help" })} />
                    </div>
                  </>
                )}
                ...
                {step === 3 && (
                  <>
                    <StepHead kicker="03 · Design" title="What should it feel like?" sub="Your visual direction. We'll bring real options to the first call." />
                    <div className="stack">
                      {DESIGNS.map((d) => (
                        <OptionCard key={d.id} active={s.design === d.id} label={d.label} desc={d.desc}
                          tag={d.amount === 0 ? "Included" : "+" + fmt(d.amount)}
                ...
                {step === 4 && (
                  <>
                    <StepHead kicker="04 · Features" title="What does it need to do?" sub="Select everything you'll need. Unsure about something? Leave it — we can add it later." />
                    <div className="grid2 feat">
                      {FEATURES.map((f) => (
                        <OptionCard key={f.id} multi active={s.features.includes(f.id)} Icon={f.Icon}
                          label={f.label} desc={f.desc}
                          tag={f.id === "multilingual" ? "set count" : f.amount === 0 ? "Included" : "+" + fmt(f.amount)}
                          onClick={() => toggleFeature(f.id)} />
                      ))}
                    </div>
                    {s.features.includes("multilingual") && (
                      <div className="stepper">
                        <span className="stepperLabel"><Globe size={15} strokeWidth={2} /> How many languages?</span>
                        <div className="stepperCtrl">
                          <button onClick={() => set({ languages: Math.max(2, s.languages - 1) })} aria-label="Fewer languages"><Minus size={15} /></button>
                          <span className="stepperVal">{s.languages}</span>
                          <button onClick={() => set({ languages: Math.min(8, s.languages + 1) })} aria-label="More languages"><Plus size={15} /></button>
                        </div>
                      </div>
                    )}
                  </>
                ...
                {step === 5 && (
                  <>
                    <StepHead kicker="05 · Timeline & budget" title="When do you need it?" sub="Timing shapes the plan — and the price." />
                    <div className="grid3">
                      {TIMELINES.map((t) => (
                        <button key={t.id} type="button" className={"tcard" + (s.timeline === t.id ? " on" : "")}
                          onClick={() => set({ timeline: t.id })} aria-pressed={s.timeline === t.id}>
                          <span className="tlabel">{t.label}</span>
                          <span className="tdesc">{t.desc}</span>
                          {t.note && <span className={"tnote" + (t.id === "asap" ? " warn" : "")}>{t.note}</span>}
                        </button>
                      ))}
                    </div>
                    <div className="subhead">
                      Your estimate lands around <strong>{fmt(est.total)}</strong>. What's your budget?
                    </div>
                    <div className="bands">
                      {BANDS.map((b) => {
                        ...
                        return (
                          <button key={b.id} type="button"
                            className={"band" + (s.budget === b.id ? " on" : "") + (isMatch ? " match" : "")}
                            onClick={() => set({ budget: b.id })} aria-pressed={s.budget === b.id}>
                            {b.label}
                            {isMatch && <span className="bandTag">matches estimate</span>}
                          </button>
                        );
                      })}
                    </div>
                ...
                {step === 6 && (
                  <>
                    <StepHead kicker="06 · Almost done" title="Where do we send your estimate?" sub="We'll reply within one business day with a fixed, itemized quote." />
                    <div className="fields">
                      <Field label="Your name" required value={s.name} onChange={(v) => set({ name: v })} placeholder="Jane Rivera" />
                      <Field label="Email" required value={s.email} onChange={(v) => set({ email: v })} placeholder="jane@company.com"
                        invalid={s.email.length > 0 && !isEmail(s.email)} note="So we can send your quote." />
                      <Field label="Business name" value={s.company} onChange={(v) => set({ company: v })} placeholder="Optional" />
                      <div className="field">
                    ...
                    <div className="reassure">
                      <CheckCircle2 size={15} strokeWidth={2.2} /> Free, no obligation. We'll never share your email or send spam.
                    </div>
                  </>
                )}
                ...
                <div className="nav">
                  ...
                  {step === 4 && s.features.length === 0 ? (
                    <button className="btn primary" onClick={next}>Skip — none of these <ArrowRight size={16} strokeWidth={2.2} /></button>
                  ) : (
                    <button className="btn primary" onClick={next} disabled={!canContinue(step, s)}>
                      {step === TOTAL_STEPS ? "Get my estimate" : "Continue"} <ArrowRight size={16} strokeWidth={2.2} />
                    </button>
                  )}
                </div>
              </div>
            ...
            <aside className="aside">
              <div className="asideSticky">
                <EstimatePanel est={est} hasType={hasType} low={low} high={high} midBand={midBand} />
              </div>
            </aside>
          </div>
          ...
          <button className="mbar" onClick={() => setShowSheet(true)}>
            <span className="mbarLeft">
              <span className="mbarKicker"><Layers size={11} strokeWidth={2.4} /> Estimate</span>
              <span className="mbarVal">{hasType ? fmt(low) + " – " + fmt(high) : "$—"}</span>
            </span>
            <span className="mbarCta">View breakdown <ArrowRight size={14} strokeWidth={2.2} /></span>
          </button>

          {showSheet && (
            <div className="sheetWrap" onClick={() => setShowSheet(false)}>
              <div className="sheet" onClick={(e) => e.stopPropagation()}>
                <button className="sheetClose" onClick={() => setShowSheet(false)} aria-label="Close"><X size={18} /></button>
                <EstimatePanel est={est} hasType={hasType} low={low} high={high} midBand={midBand} compact />
              </div>
            </div>
          )}
        </>
...
function Field({ label, required, value, onChange, placeholder, note, invalid }) {
  return (
    <div className="field">
      <label>{label} {required ? <span className="req">*</span> : <span className="opt">Optional</span>}</label>
      <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
        className={invalid ? "bad" : ""} />
      {invalid ? <span className="fieldErr">Enter a valid email so we can reach you.</span>
...
function Confirmation({ s, est, low, high, onRestart }) {
  ...
  const chips = [
    type && type.label,
    design && design.label,
    s.features.length ? s.features.length + " feature" + (s.features.length > 1 ? "s" : "") : null,
  ...
  return (
    <div className="done">
      <div className="doneBadge"><CheckCircle2 size={30} strokeWidth={2} /></div>
      <h2>Your plan is ready, {first}.</h2>
      <p className="doneSub">Here's the shape of your project. We've sent a copy to your inbox and we'll follow up within one business day.</p>

      <div className="doneEst">
        <div className="doneEstKicker"><Sparkles size={13} strokeWidth={2.2} /> Estimated investment</div>
        <div className="doneEstBig">{fmt(low)} – {fmt(high)}</div>
        <div className="doneEstNote">Itemized and fixed once we confirm scope on a short call.</div>
      </div>
      ...
      <div className="next">
        ...
        <ol className="nextList">
          <li><span className="nstep">1</span><div><strong>We review your plan.</strong> Within one business day.</div></li>
          <li><span className="nstep">2</span><div><strong>You get a fixed quote.</strong> Itemized, no surprises.</div></li>
          <li><span className="nstep">3</span><div><strong>We book a short call.</strong> To lock scope and timeline.</div></li>
        </ol>
      </div>

      <div className="doneFoot">
        Questions in the meantime? Email <a href="mailto:james@dr-webstudio.com">james@dr-webstudio.com</a>.
      </div>
      <button className="btn ghost restart" onClick={onRestart}><ArrowLeft size={15} strokeWidth={2.2} /> Start over</button>
    </div>
  );
}
