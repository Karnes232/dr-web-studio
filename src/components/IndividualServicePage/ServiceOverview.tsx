...
const ServiceOverview = ({ title, longDescription, service }: { title: string, longDescription: string, service: any }) => {
    const { t } = useLocale()
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <ul className="space-y-2 text-gray-700">
              {service.beforeState.map((item: string, index: number) => (
          ...
          <div>
            ...
            <ul className="space-y-2 text-gray-700">
              {service.afterState.map((item: string, index: number) => (
                <li key={index} className="flex items-start">
